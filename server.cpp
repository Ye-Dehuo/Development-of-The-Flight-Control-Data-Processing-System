...
std::string generateFileName(const unsigned long& timestamp) {
   ...
   unsigned long timestamp_s = timestamp / 1000; // 时间戳单位ms转换为s
    ...
    std::tm *ptm = std::localtime(&time);
     ...
     // 使用std::ostringstream与std::put_time来格式化时间
...
void receiveDataFromClient(int newSock, std::chrono::high_resolution_clock::time_point
 start, int processCount) {
    ...
    bool clientDisconnected = false; // 记录客户端是否断开
    ...
    while (true) {
        ...
        dataStrings.clear();
        clientDisconnected = true; // 标记客户端断开
        break;
        ...
        std::cout << dataStrings.size() << " set of data has been received" << std::endl; // 打印本批数据已接收数量
        if (dataStrings.size() == 10) std::cout << "This batch of data has been received" << std::endl << std::endl;
    }

    if (clientDisconnected) break;  // 退出最外层循环
